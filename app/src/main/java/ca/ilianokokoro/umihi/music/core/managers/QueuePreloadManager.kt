 ...
 *   1. Resolves + caches the stream URL for the next [PRELOAD_AHEAD] songs via [YoutubeHelper.getSongPlayerUrl]
 *      (which saves to the Room DB so YoutubeDataSourceFactory finds it without a network call).
 *   2. Downloads + saves the album art to the thumbnail directory so PlaybackService
 *      finds the local file immediately on transition.
 *
